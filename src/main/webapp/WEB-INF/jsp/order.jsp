				...
				<form id="form-order">
					<div class="box-body">
						...
						<div class="row">
							<div class="col-md-3">
								<div class="form-group">
									<label>Kode Transaksi</label> <input type="text"
							...
							<div class="col-md-6">
								<div class="form-group">
									<label>Nama Pelanggan</label> <input type="text"
										class="form-control" name="namaPelanggan" id="namaPelanggan">
								</div>
							</div>
							<div class="col-md-3">
								<div class="form-group">
						...
						<div class="row">
							...
							<div class="col-md-6">
								<div class="form-group">
									<label>Barang</label> <select class="form-control"
										name="barang" id="barang"></select>
								</div>
							</div>
							<div class="col-md-3">
								<div class="form-group">
									<label>Jumlah barang</label> <input type="number"
										class="form-control" name="jumlahBarang" id="jumlahBarang"
										value="1">
								</div>
							</div>
						</div>
					...
					<div class="box-footer">
						<button type="button" class="btn btn-primary"
							onclick="tambahOrderDetail()">Tambah Barang</button>
					</div>
				</form>
			</div>
		</div>
			...
			<div class="box">
				...
				<div class="box-footer">
					<div class="row" style="display: flex; align-items: flex-end;">
						<div class="col-xs-8">
							<button type="button" class="btn btn-primary"
								onclick="simpanOrder()">Submit Order</button>
						</div>
						<div class="col-xs-4">
							<label>Grand Total</label>
							<div class="input-group">
								<span class="input-group-addon">Rp</span> <input type="number"
									class="form-control" name="grandTotal" id="grandTotal" readonly>
							</div>
						</div>
					</div>
				</div>
	...
	function tambahOrderDetail() {
		...
		$.each(orderDetails, function(index, element) {
			if (element.id.kodeBarang == $('#barang').val()) {
				element.jumlahBarang = Number(element.jumlahBarang)
						+ Number($('#jumlahBarang').val());
		...
		if (orderDetailBaru) {
			...
			orderDetail.hargaSatuan = $('#barang > option:selected').data(
					'harga');
			orderDetail.h_namaBarang = $('#barang > option:selected').data(
					'nama');
			// tanpa subtotal
	...
	function tampilkanData(d) {
		...
		$(d)
          ...
          + '" data-harga="' + element.harga + '">'
									...
									+ ' - '
									+ element.nama
									+ ' - Rp' + element.harga + '</option>';
						});
		$('#barang').html(s);
	}
	// fungsi utk load & tampilkan data order
	...
	function tampilkanDataOrder(d) {
		...
		$(d).each(
				function(index, element) {
					tabelOrder.row.add(
							[
									'<a href="order/' + element.kode + '">'
											+ element.kode + '</a>',
									element.namaPelanggan,
									element.tglTransaksi, element.grandTotal ])
							.draw();
				})
	}
	// document.ready
